...
class streams extends CI_Controller
{
  ...
  public function index()
  {
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title col-md-4">' . $string['stream'] . '</h4>
                  ' . $this->admin_model->get_page_header() . $this->admin_model->get_page_button('streams', true, true, true) . '
                </div>
                <div class="card-body">
    ...
    $html = '
    <form method="post" action="' . base_url() . 'admin/streams/add" enctype="multipart/form-data">
     ...
     ';
    $content .= $this->modal_model->dialog($html, 'streams_panel', 'md', $string['save']);


    $html = '<form method="post" action="' . base_url() . 'admin/streams/settings" enctype="multipart/form-data">
    ...
    $content .= $this->modal_model->dialog($html, 'streams_settings', 'md', $string['save']);
  ...
  public function read()
  {
    $content = '';
    $n = 0;
    $ar = $this->database_model->read('streams');
  ...
  public function read_row()
  {
    ...
    if (!empty($id) and $id != 0) {
      $result = $this->database_model->read_row('streams', $id);
  ...
  public function add()
  {
    ...
    if (!empty($title)) {
      ...
      if (empty($ch_id)) {
        $ar = array_merge($ar, array('stream_id' => $stream_id));
        $this->database_model->insert('streams', $ar);
      } else {
        $this->database_model->update('streams', $ar, $ch_id);
      }
  ...
  public function delete()
  {
    $id = $this->input->post('id');
    if (!empty($id) and $id != 0) {
      $this->database_model->delete('streams', $id);
  ...
  public function copy_row()
  {
    $id = $this->input->post('id');
    if (!empty($id) and $id != 0) {
      $column = array('share', 'stream_id', 'l_id', 'title', 'num');
      $this->database_model->copy_row('streams', $column, $id);
